...
`pg_upgrade` replaces the old cluster's data files with the new version in place, without dumping and reloading data. It supports two modes:
...
# Parallel checking (PG18+): speed up pre-checks
pg_upgrade ... --jobs 4
```
...
pg_upgrade generates helper scripts in the current directory after a successful run:

```bash
# Generated by pg_upgrade — analyze all databases (update optimizer statistics)
...
PG18's `pg_upgrade` preserves optimizer statistics by default, eliminating the post-upgrade performance dip while `ANALYZE` runs:
...
On older versions, always run `ANALYZE` on all databases immediately after upgrading.
...
pg_createsubscriber \
    ...
    --publisher-conninfo "host=old_primary dbname=mydb" \
    --all
```
...
```bash
# Run the generated script (analyzes all databases)
./analyze_new_cluster.sh
...
On PG18+ with `pg_upgrade`, statistics are preserved by default — this step is optional but still recommended after significant schema changes.
...
- **pg_upgrade (swap mode)**: Run `pg_upgrade --swap` again to reverse the swap
